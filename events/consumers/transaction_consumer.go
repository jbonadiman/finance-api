		...
		go func() {
			err = rabbitCon.RegisterConsumer(
				...
				func(msg <-chan amqp.Delivery) {
					for event := range msg {
						log.Printf("received an event of id %q\n", event.Timestamp)
						task := models.Task{}

						err := json.Unmarshal(event.Body, &task)
						if err != nil {
							log.Printf("unmarshalling of event %v resulted in the following error: %v\n", event, err)
						}

						values := strings.Split(task.Title, ";")

						cost, err := strconv.ParseFloat(strings.TrimSpace(values[0]), 64)
						...
						if err != nil {
							log.Printf("could not store the task of ID %v due to the error: %v\n", task.Id, err.Error())
						} else {
							log.Printf("transaction %v created successfully!\n", id)

							err = event.Ack(true)
							if err != nil {
								log.Printf("could not acknowledge RabbitMQ event: %v\n", err)
							}
						}
					}
				})
